...
describe('O-CREAM-v2 Ontology', () => {
  describe('DOLCE Categories', () => {
    test('should define all DOLCE foundational categories', () => {
      ...
      expect(DOLCECategory.AGENTIVEPHYSICALOBJECT).toBe('AgentivePhysicalObject');
      expect(DOLCECategory.NONAGENTIVEPHYSICALOBJECT).toBe('NonAgentivePhysicalObject');
    });
  });
  ...
  describe('Relationship Module', () => {
    test('should define comprehensive relationship types', () => {
      ...
      expect(RelationshipType.CUSTOMERRELATIONSHIP).toBe('CustomerRelationship');
      expect(RelationshipType.SUPPLIERRELATIONSHIP).toBe('SupplierRelationship');
      expect(RelationshipType.PARTNERRELATIONSHIP).toBe('PartnerRelationship');
      expect(RelationshipType.PROSPECTRELATIONSHIP).toBe('ProspectRelationship');
  ...
  describe('Knowledge Module', () => {
    test('should define comprehensive knowledge types', () => {
      // Customer knowledge
      expect(KnowledgeType.CUSTOMERPROFILE).toBe('CustomerProfile');
      expect(KnowledgeType.CUSTOMERPREFERENCES).toBe('CustomerPreferences');
      expect(KnowledgeType.CUSTOMERBEHAVIOR).toBe('CustomerBehavior');
      expect(KnowledgeType.CUSTOMERHISTORY).toBe('CustomerHistory');
      
      // Business knowledge
      expect(KnowledgeType.MARKETINTELLIGENCE).toBe('MarketIntelligence');
      expect(KnowledgeType.COMPETITIVEANALYSIS).toBe('CompetitiveAnalysis');
      expect(KnowledgeType.PRODUCTKNOWLEDGE).toBe('ProductKnowledge');
      expect(KnowledgeType.PROCESSKNOWLEDGE).toBe('ProcessKnowledge');
      
      // Transactional knowledge
      expect(KnowledgeType.TRANSACTIONDATA).toBe('TransactionData');
      expect(KnowledgeType.INTERACTIONHISTORY).toBe('InteractionHistory');
      expect(KnowledgeType.COMMUNICATIONLOG).toBe('CommunicationLog');
    });
    ...
    test('should create information element with all properties', () => {
      const info: InformationElement = {
        ...
        type: KnowledgeType.CUSTOMERPROFILE,
      ...
      expect(info.type).toBe(KnowledgeType.CUSTOMERPROFILE);
  ...
  describe('Activity Module', () => {
    test('should define comprehensive activity types', () => {
      ...
      // Sales activities
      expect(ActivityType.LEADQUALIFICATION).toBe('LeadQualification');
      expect(ActivityType.OPPORTUNITYMANAGEMENT).toBe('OpportunityManagement');
      expect(ActivityType.PROPOSALPREPARATION).toBe('ProposalPreparation');
      expect(ActivityType.NEGOTIATION).toBe('Negotiation');
      expect(ActivityType.CLOSING).toBe('Closing');
      
      // Marketing activities
      expect(ActivityType.CAMPAIGNEXECUTION).toBe('CampaignExecution');
      expect(ActivityType.CONTENTCREATION).toBe('ContentCreation');
      expect(ActivityType.MARKETRESEARCH).toBe('MarketResearch');
      expect(ActivityType.BRANDMANAGEMENT).toBe('BrandManagement');
      
      // Service activities
      expect(ActivityType.CUSTOMERSUPPORT).toBe('CustomerSupport');
      expect(ActivityType.ISSUERESOLUTION).toBe('IssueResolution');
      expect(ActivityType.MAINTENANCE).toBe('Maintenance');
      expect(ActivityType.TRAINING).toBe('Training');
      
      // Information management activities
      expect(ActivityType.DATACOLLECTION).toBe('DataCollection');
      expect(ActivityType.DATAANALYSIS).toBe('DataAnalysis');
      expect(ActivityType.REPORTING).toBe('Reporting');
      expect(ActivityType.DOCUMENTMANAGEMENT).toBe('DocumentManagement');
    });
    ...
    test('should create comprehensive CRM activity', () => {
      const activity: CRMActivity = {
        ...
        type: ActivityType.OPPORTUNITYMANAGEMENT,
      ...
      expect(activity.type).toBe(ActivityType.OPPORTUNITYMANAGEMENT);
  ...
  describe('Software Module', () => {
    test('should define software types', () => {
      expect(SoftwareType.CRMPLATFORM).toBe('CRMPlatform');
      expect(SoftwareType.SALESAUTOMATION).toBe('SalesAutomation');
      expect(SoftwareType.MARKETINGAUTOMATION).toBe('MarketingAutomation');
      expect(SoftwareType.EMAILSYSTEM).toBe('EmailSystem');
      expect(SoftwareType.KNOWLEDGEGRAPH).toBe('KnowledgeGraph');
    ...
    test('should create software system entity', () => {
      ...
      expect(software.category).toBe(DOLCECategory.NONAGENTIVEPHYSICALOBJECT);
      expect(software.type).toBe(SoftwareType.CRMPLATFORM);
  ...
  describe('OCreamV2Ontology Manager', () => {
    ...
    test('should add and retrieve entities', () => {
      ...
      const retrievedEntity = ontology.getEntity('test-1');
      
      expect(retrievedEntity).toBeDefined();
      expect(retrievedEntity?.id).toBe('test-1');
      expect(retrievedEntity?.category).toBe(DOLCECategory.ABSTRACT);
    });
    ...
    test('should manage relationships', () => {
      const entity1: DOLCEEntity = { id: 'e1', category: DOLCECategory.AGENTIVEPHYSICALOBJECT, createdAt: new Date(), updatedAt: new Date() };
      const entity2: DOLCEEntity = { id: 'e2', category: DOLCECategory.AGENTIVEPHYSICALOBJECT, createdAt: new Date(), updatedAt: new Date() };
      ontology.addEntity(entity1);
      ontology.addEntity(entity2);
    ...
    test('should find related entities', () => {
      const entity1: DOLCEEntity = { id: 'e1', category: DOLCECategory.AGENTIVEPHYSICALOBJECT, createdAt: new Date(), updatedAt: new Date() };
      const entity2: DOLCEEntity = { id: 'e2', category: DOLCECategory.AGENTIVEPHYSICALOBJECT, createdAt: new Date(), updatedAt: new Date() };
      const entity3: DOLCEEntity = { id: 'e3', category: DOLCECategory.AGENTIVEPHYSICALOBJECT, createdAt: new Date(), updatedAt: new Date() };
      ontology.addEntity(entity1);
      ontology.addEntity(entity2);
      ontology.addEntity(entity3);
    ...
    test('should update an existing entity', () => {
      const initialEntity: DOLCEEntity = {
        id: 'entity-to-update',
        category: DOLCECategory.QUALITY,
        createdAt: new Date('2024-01-01'),
        updatedAt: new Date('2024-01-01')
      };
      ontology.addEntity(initialEntity);

      const updatedEntityData = {
        category: DOLCECategory.ABSTRACT // Change category
      };
      ontology.updateEntity('entity-to-update', updatedEntityData);

      const retrieved = ontology.getEntity('entity-to-update');
      expect(retrieved?.category).toBe(DOLCECategory.ABSTRACT);
      expect(retrieved?.updatedAt).not.toEqual(initialEntity.updatedAt);
    });

    test('should not throw when updating non-existent entity', () => {
      expect(() => {
        ontology.updateEntity('non-existent', { category: DOLCECategory.ABSTRACT });
      }).not.toThrow();
    });
  });
