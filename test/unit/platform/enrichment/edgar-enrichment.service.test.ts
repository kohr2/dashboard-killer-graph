...
// Define a minimal Organization type for testing, avoiding path issues.
interface Organization extends EnrichableEntity {
  label: 'Organization';
  legalName: string;
}
...
describe('EdgarEnrichmentService', () => {
  ...
  describe('enrich', () => {
    ...
    const baseEntity: Organization = {
        id: 'org-1',
        name: 'Enrichment Corp',
        label: 'Organization',
        legalName: 'Enrichment Corporation',
    };
    ...
    it('should return null for an unknown organization', async () => {
      mockedFs.readFile.mockResolvedValue(JSON.stringify(mockCikData));
      const unknownEntity: Organization = { ...baseEntity, name: 'Unknown LLC' };
      const result = await service.enrich(unknownEntity);
      expect(result).toBeNull();
    });
    
    it('should return null and log an error if initialization fails', async () => {
    ...
    it('should not enrich entities that are not Organizations', async () => {
        const personEntity: EnrichableEntity = { id: 'p-1', name: 'John Doe', label: 'Person' };
