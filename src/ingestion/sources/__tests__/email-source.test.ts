...
import { EmailSource, EmailSourceConfig } from '../email-source';
import { SourceType } from '../../types/data-source.interface';
...
describe('EmailSource', () => {
  ...
  beforeEach(() => {
    config = {
      ...
      enabled: true,
      provider: 'eml',
      directory: path.join(process.cwd(), 'test', 'fixtures', 'emails')
    };
  });
  ...
  describe('fetch', () => {
    ...
    it('should throw error for unsupported provider', async () => {
      ...
      await expect(iterator.next()).rejects.toThrow('Provider exchange not implemented yet');
    });
