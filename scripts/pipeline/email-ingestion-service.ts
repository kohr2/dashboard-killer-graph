...
let INDEXABLE_LABELS: string[] = [];

export class EmailIngestionService {
  private pipeline: GenericIngestionPipeline;
  ...
  constructor() {
    ...
    this.pipeline = new GenericIngestionPipeline(
      ...
      {
        ingestEntitiesAndRelationships: this.ingestEntitiesAndRelationships.bind(this)
      }
    );
  }
  ...
  async initialize(): Promise<void> {
    ...
    const validRelationshipTypes = this.ontologyService.getAllRelationshipTypes();
    INDEXABLE_LABELS = this.ontologyService.getIndexableEntityTypes();
  ...
  async processEmails(testEmailsDir: string): Promise<void> {
    ...
    await this.pipeline.run(emailInputs);
    
    console.log('[2] Batch processing complete.');
  }

  private async ingestEntitiesAndRelationships(result: ProcessingResult): Promise<void> {
    if (!this.session) {
    ...
    for (const entity of nonPropertyEntities) {
      ...
      if (!foundExistingNode) {
        ...
        // Add property values if this entity has associated properties
        ...
        // Merge enriched data using utility to flatten nested structures
        ...
        // Existing node matched – update its properties
        const entityProperties = propertyRelationships.get(entity.id) || new Map();
        ...
        // Only run update if we have at least one property to set
    ...
    // Only link non-property entities to communication
    for (const entity of nonPropertyEntities) {
    ...
    // 3. Create relationships between entities (excluding property relationships)
